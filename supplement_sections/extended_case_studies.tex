\section{Extended Cross-Domain Case Studies}

The main paper keeps only compressed case-study lessons. This section restores fuller deployment-shaped reasoning while preserving the same five-field structure.

\subsection{Multi-Tenant LLM Serving Under Burst Arrival}

\paragraph{State object.}
The governed objects are paged KV state, prefix-sharing metadata, tenant-specific adapters, model-residency windows, and restoration debt inherited by subsequent requests.

\paragraph{Control surface.}
The runtime controls admission, batch shaping, paging, prefill/decode separation, transfer, reclamation, restoration, and tenant-sensitive placement~\cite{kwon2023pagedattention,agrawal2024sarathi,zhong2024distserve,patel2024splitwise,chen2024punica,nian2026cacheflow}.

\paragraph{Coupling path.}
A locally beneficial admission may force reclamation that destroys future reuse, causing later restoration traffic and queue amplification. Once phase disaggregation is enabled, the next bottleneck may shift from raw memory capacity to state transfer and restoration ordering.
...
The unresolved problem is still a unified contract that ranks admission, transfer, offload, tenant isolation, and restoration against the same service boundary instead of letting each sub-policy optimize its own phase locally.
...
\paragraph{Remaining gap.}
The missing abstraction is an exposure-aware accounting layer that can combine recall drift, rebuild debt, planner confidence, and publication legality into one auditable trigger policy.

...
The survey applies the same five-question scaffold whenever it summarizes a system line: what is the \emph{state object}; what \emph{control surface} does the runtime expose; what \emph{coupling path} makes that surface important; what \emph{evaluation boundary} is actually optimized; and what \emph{remaining systems gap} is left open. This scaffold gives the survey a stable comparison unit across otherwise different domains and helps prevent a common failure mode in survey writing: turning papers into disconnected summaries of implementation details.
...
To make the propagation claim falsifiable rather than merely aspirational, we trace one concrete decision through all three axes using Example~B (multi-tenant LLM serving) with representative numbers drawn from the vLLM~\cite{kwon2023pagedattention} and Sarathi-Serve~\cite{agrawal2024sarathi} literature. The detailed trace is useful as an extended illustration, but it is not required for the main argumentative thread. We therefore keep only a short summary here and leave the longer walk-through to the supplement.

\smallskip\noindent\textbf{Summary.} Under bursty multi-tenant load, an admission decision that looks locally beneficial can trigger KV eviction, destroy prefix reuse, increase TTFT, and create restoration debt that feeds back into future access pressure. In five-tuple terms, the state object is shared KV-cache state; the control surface is admission plus eviction; the coupling path is prefix-reuse destruction; the evaluation boundary is latency and goodput; and the remaining gap is that most serving stacks still lack a lifecycle-closed controller over admission, transfer, reclamation, and restoration. A full numerical trace can therefore live in the supplement without weakening the main textual argument here.
The worked example shows that admission, execution, and evolution form one coupled loop: an access decision that looks locally efficient can raise later execution cost and create restoration debt that remains invisible in an access-only view. The key systems lesson is not the exact numeric trace, but the fact that eviction debt should be charged against future reuse loss rather than against instantaneous throughput alone. The supplement keeps the long-form evidence, while the main text retains the comparative lesson.
...
These rules also imply an evidence hierarchy. A paper that makes a seam visible is not yet equivalent to one that shows the seam remains governable under disturbance, multi-tenancy, and lifecycle interaction. The synthesis below therefore emphasizes transferable mechanisms and stable open seams, while the supplement carries the fuller matrices and extended evidence.
...
The remaining gap is again a contract gap. Retrieval systems still lack a compositional trigger policy that orders stale-memory tolerance, rebuild or compaction triggers, shard-exposure legality, and page or fabric movement under one auditable service boundary. Until that contract becomes explicit, reported gains will remain hard to compare because some systems optimize request-time quality, others optimize maintenance stability, and others optimize exposure safety or placement efficiency.


Seen from that angle, the remaining gaps are not isolated implementation chores but missing governance semantics across layers: stale-memory tolerance, rebuild triggers, exposure legality, and maintenance accounting still lack one auditable service boundary.
...
Serving memory, retrieval memory, and retention stores have different lifetimes, but they all revolve around the same governance pattern.
...
A first architectural choice is the granularity at which state is represented and controlled. Record, key, segment/page, and session/conversation granularities trade flexibility, locality, and metadata overhead differently~\cite{zhang2019briskstream,kwon2023pagedattention,zheng2023sglang}.

\subsubsection{Control-Plane Coupling: Embedded, Sidecar, or Shared Service}

A second choice is where control logic lives. Embedded control is fast but fragmented, sidecar-style control is modular but delayed, and shared services are consistent but can bottleneck if their APIs are too fine-grained~\cite{zhang2020concurrent,kwon2023pagedattention,zhang2026flowrag}.

\subsubsection{Lifecycle Management: Admit, Place, Mutate, Compact, Evict}

A robust stateful runtime should make five lifecycle stages explicit: admit, place, mutate, compact, and evict. If any stage is implicit, local optimizations tend to create lifecycle debt under disturbance~\cite{sheng2023slora,zhang2025jenga,yu2025prism}.
...
A useful way to read the literature is that stronger systems approximate a machine-readable state catalog containing state object names, granularity, mutability class, lifecycle stage, and quality sensitivity. This is the minimum condition for comparing access, execution, and reuse decisions against the same state object.

\subsubsection{Observation and Disturbance Detection}

Useful observation layers combine steady metrics with disturbance indicators such as hotspot flips, phase imbalance, retrieval drift, and fragmentation spikes. The common pattern is that a paper becomes more auditable once the disturbance it reacts to is named explicitly rather than inferred after the fact.

\subsubsection{Boundary Model and Decision Kernel}

The decision kernel is where state catalog and observation streams become boundary-aware choices. Whether heuristic, model-based, or learned, the comparative requirement is the same: decisions should remain auditable and their precedence over competing objectives should be explicit.
...
The design implications above can be distilled into four pressure points: cross-domain observability contracts, state-centric scheduling on heterogeneous hardware, memory middleware for dynamic AI services, and end-to-end evaluation beyond local speedups~\cite{zhang2019briskstream,zhang2020finestream,kwon2023pagedattention,sheng2023flexgen,cong2026mico,zhang2026neuromem,liu2026sage}. In each case the missing piece is the same: a shared control layer that makes state identity, movement cost, exposure legality, and lifecycle debt composable under one service boundary.

\section{Conclusion}
\label{sec:conclusion}

This survey reframes efficient state management as one coupled control loop over access, execution, and evolution. Normalizing streaming, serving, retrieval, and retention to the same five-tuple shows why the recurring issue is governance contracts, not isolated optimizations. The field now needs typed state catalogs, bounded-delay telemetry, and policy composition that survives disturbance.

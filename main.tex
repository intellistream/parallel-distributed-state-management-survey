...
MOTTO is especially relevant to this survey because it makes explicit what the operative state is in pattern-centric stream processing. The state is not merely a relation or key-value table; it consists of window-resident events, partial matches accumulated during pattern evaluation, and reusable subquery results that can be consumed by multiple concurrent pattern queries. MOTTO's contribution is to show that this state can be shared at multiple granularities: whole-query reuse through merge sharing, finer-grained reuse through decomposition into subqueries, and expanded reuse through operator transformation across equivalent pattern forms. The handling of nested queries and heterogeneous windows is particularly important from a state-management perspective, because it demonstrates that safe sharing depends on preserving the temporal validity of partial matches rather than only detecting syntactic overlap among queries.

This makes MOTTO more than a classic query-optimization paper. It is an early example of state-aware access governance: the optimizer rewrites the workload so that multiple consumers can read from common runtime state while filters and window checks prevent stale or semantically invalid reuse. At the same time, the paper also exposes a boundary that recurs throughout later systems work: sharing is profitable only when the cost of coordinating and filtering shared state remains below the cost of recomputation. In that sense, MOTTO anticipates a central survey theme, namely that state management is fundamentally about deciding when shared state should be exposed, how it should be structured for reuse, and when the resulting access path becomes more expensive than independent execution.

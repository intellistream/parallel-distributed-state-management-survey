...
\begin{abstract}
State has become a major source of both performance leverage and system fragility across streaming, serving, retrieval, and continual-learning systems. Yet the literature remains split across access control, hardware-conscious execution, memory management, and long-horizon updates. This survey asks how runtimes manage shared, evolving state under concurrency, heterogeneous hardware, and non-stationary workloads. We organize work around three coupled dimensions: state access and scheduling, state-aware execution, and state evolution and reuse. A common tuple links local mismatches to system-level instability. The synthesis yields cross-domain comparisons, a contract-oriented blueprint, recurring anti-patterns, and a disturbance-oriented evaluation agenda. Overall, state is a runtime control problem.
\end{abstract}
...
Systems abstractions have shifted from stateless throughput engines toward long-running services governed by evolving state. Foundational streaming systems already made state and time runtime concerns~\cite{akidau2013millwheel,murray2013naiad,zaharia2013dstreams,akidau2015dataflow}; newer LLM serving and retrieval systems expose the same pressure through KV caches, scheduling metadata, vector memories, and retained update traces~\cite{kwon2023pagedattention,agrawal2024sarathi,zheng2023sglang,zhou2025ferret,zhang2026flowrag}. Across these settings, the decisive systems question is whether the runtime can keep shared, long-lived, performance-critical state governable without losing service stability.

This evolution makes \sysproblem harder to reason about. Traditional decompositions study operator placement, memory layout, or model adaptation separately, but deployed systems couple them: access hotspots reshape queues and locality, kernel-level gains vanish under data movement or quality constraints, and aggressive memory updates destabilize retention or retrieval downstream. The problem is not optimization in one phase, but control over a lifecycle in which local gains reappear as later constraints.

We use \emph{state management} to mean the mechanisms that organize, observe, access, update, retain, and reuse shared state across execution boundaries. The concrete object may be a shared hash table, compression dictionary, KV cache, or retriever memory. In each case, the runtime must expose the object, price its access cost, bind it to execution policy, and preserve future reuse value.

We organize the literature around three dimensions that are analytically distinct but operationally coupled: access decisions reshape execution, execution choices change update cost, and evolution policies alter future access.
...
The survey makes four contributions. It provides a vocabulary for comparing streaming, approximate execution, serving, retrieval, and memory-centric intelligent services; develops a three-axis taxonomy around access, execution, and evolution; extracts recurring anti-patterns and a contract-oriented blueprint for stateful runtimes; and proposes disturbance-aware evaluation boundaries for mechanisms whose local gains may not survive composition. The emphasis is comparative rather than exhaustive.
...
\end{itemize}
These criteria favor papers that make control decisions legible. For retrieval and learning lines, model-centric papers enter the analytical core only when they expose managed memory objects, planner-visible controls, or maintenance boundaries.

\subsection{Survey Protocol and Evidence Posture}

The review protocol is comparative and mechanism-centered rather than narrowly bibliometric. We seed the corpus from canonical systems lines and adjacent surveys, then follow forward and backward citation paths around shared-state access, state-aware execution, and state evolution. The aim is to cover mechanism families, not to maximize paper counts. When a stable venue version exists, we prefer it over a moving preprint; recent papers enter the analytical core only when they expose a new state object, runtime control seam, or evaluation boundary. The synthesis distinguishes foundational papers, transfer-oriented systems, and frontier systems, so local speedups, transfer demonstrations, and long-horizon governance claims are not treated as equivalent evidence.

The protocol also separates \emph{coverage} from \emph{representativeness}. Coverage asks whether the survey has included the mechanism families needed to support its taxonomy: visibility and progress, ownership transfer, hardware-conscious placement, approximation control, serving-memory lifecycle management, retrieval maintenance, and retention governance. Representativeness asks whether the selected papers expose enough design variation to make comparison meaningful. For example, a single KV-cache allocator would not support claims about serving-memory lifecycle control; the survey therefore compares paging, prefix reuse, phase separation, tenant multiplexing, disaggregation, and restoration mechanisms under one latency and memory boundary. Similarly, one vector-index paper would not support claims about retrieval evolution; the survey instead combines retriever invocation, mutable ANN maintenance, publication safety, freshness control, and placement.

This distinction matters because the paper is not trying to be an encyclopedic bibliography. A paper can be important historically yet remain contextual if it does not expose a runtime control surface over state; conversely, a recent system can enter the analytical core if it makes a previously hidden lifecycle boundary measurable. Borderline papers are handled by asking whether they change at least one field of the comparison tuple. A storage-engine paper, for instance, may remain a boundary case when it assumes stable object identities and clear invalidation semantics, but it becomes analytically relevant when compaction, admission, eviction, or tiering becomes visible to a scheduler or service-level controller. This rule keeps the scope broad enough to connect streaming, serving, retrieval, and retention, while preventing the survey from collapsing into a list of all systems that merely store data.

The evidence classes used later in the paper follow from this rule. \emph{Foundational} papers introduce state objects or control boundaries that later systems inherit, such as progress frontiers, checkpoint barriers, ownership metadata, bounded-error summaries, or reusable memory stores. \emph{Transfer-oriented} papers show that a mechanism survives a neighboring systems pressure: a placement policy remains useful under recovery, a cache policy remains useful under tenant churn, or a retrieval policy remains useful under update drift. \emph{Frontier} papers expose a control boundary that is not yet mature but is likely to matter as workloads change, such as disaggregated serving memory, planner-visible retrieval freshness, or budget-elastic retention. We use these classes to avoid a common survey failure mode: treating an early abstraction paper, a mature runtime system, and a speculative frontier system as if they made the same kind of claim. The comparison is strongest when it states which class a paper belongs to and what kind of evidence would be needed to move the mechanism into a stronger class.

We also treat negative space as evidence. When a paper reports excellent local performance but omits migration, recovery, freshness, restoration, or quality-debt measurements that its own mechanism would plausibly affect, that omission becomes part of the comparative result. This does not make the paper weak in its own domain; it marks the boundary beyond which its evidence should not be extrapolated. For example, an LLM serving paper that improves goodput through aggressive reclamation still leaves open whether the reclaimed state damages prefix reuse under follow-up bursts. A retrieval paper that improves recall-latency trade-offs on a static index still leaves open whether deletion repair and publication lag change the service boundary. These omissions are precisely where the survey's unresolved-contract field becomes useful.
...
The recurring control problems fall into three coupled dimensions: state access and scheduling, state-aware execution, and state evolution and reuse. A local gain along one dimension often reappears as a downstream constraint on the other two. We use three running examples to keep the comparison grounded: (A) a streaming fraud detector under hotspot skew, (B) a multi-tenant LLM assistant under KV-memory pressure, and (C) a RAG knowledge service under corpus drift.

\subsection{State Access and Scheduling}

\subsubsection{From Invisible Contention to Observable State Access}

In running example A, a fraud-detection pipeline suddenly receives a skewed burst after a campaign launch: the model logic is unchanged, but a small set of hot keys now serializes queue and lock paths. The practical question is not ``can the operator run faster,'' but ``can the runtime observe and reshape shared access before the skew hardens into replay and migration debt.'' As multicore stream and event systems began to scale, this problem became hard to ignore: operator logic itself was often not the main bottleneck, because hidden contention in shared access paths could dominate end-to-end behavior. Studies of multicore stream processing and complex event processing made this visible by showing that shared queues, synchronization patterns, fine-grained operator interactions, and sub-computation sharing can all limit scalability when the runtime cannot see where interference is forming~\cite{zhang2017revisiting,zhang2017mqo}. State management begins with observability, but access metrics matter only when they preserve enough structural context for the runtime to decide whether the right response is repartitioning, migration, admission throttling, or a change in execution grain.
...
Returning to Example~A, the fraud-detection runtime has observed a hotspot. The next question is how expensive it would be to repartition or migrate the hot keys given the current NUMA topology, checkpoint state, and downstream dependencies. This is a cost-modeling problem, not merely a detection problem. Once access patterns become observable, the next challenge is pricing them correctly under skew and topology. Topology-aware and concurrent stateful streaming systems showed that hotspot placement, NUMA distance, synchronization design, state partitioning, and task assignment must be considered jointly; locality decisions that ignore any one of these factors can erase the gains of parallelization~\cite{zhang2019briskstream,zhang2020concurrent}.

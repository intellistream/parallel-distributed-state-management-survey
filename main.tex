...
The supplementary evidence is useful here because it separates three families that are easy to blur under the phrase ``KV-cache optimization.'' Predictive-serving systems forecast different future debts: phase imbalance, future KV peaks, heterogeneous placement pressure, or queue amplification. They are valuable only when the prediction is tied to an action surface such as admission, batching, re-sharding, offload, or transfer. Structural-reuse systems ask a different question: not how to shrink a cache entry, but whether a prefix, branch, chunk, adapter state, or session fragment may be reused legally. Temporal-lifecycle systems intervene at yet another boundary, preserving or restoring reuse value across turns, tenants, cold starts, and disaggregated decode pools. Grouped this way, the serving literature forms one lifecycle---admit, place, share, reclaim, transfer, restore---rather than a collection of unrelated cache tricks.
...
\begin{center}
\small
\refstepcounter{table}\label{tab:disturbance-evaluation-matrix}
\textbf{Table~\thetable. Representative disturbance-oriented evaluation matrix.}\par\smallskip
